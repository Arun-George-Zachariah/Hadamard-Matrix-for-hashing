...
def get_hollywood2(data_root='./dataset/Hollywood2',
               clip_length=8,
               train_interval=2,
               val_interval=2,
               mean=[0.485, 0.456, 0.406],
    ...
    val_sampler   = sampler.SequentialSampling(num=clip_length,
                                               ...
                                               shuffle=True)
    val   = VideoIter(video_prefix=os.path.join(data_root, 'raw', 'data'),
                      txt_list=os.path.join(data_root, 'raw', 'list_cvt', 'test.txt'),
                      sampler=val_sampler,
                      force_color=True,
                      video_transform=transforms.Compose([
                                         transforms.Resize((256, 256)),
...
def get_msr_vtt(data_root='./dataset/MSR_VTT',
               clip_length=8,
               train_interval=2,
               val_interval=2,
               mean=[0.485, 0.456, 0.406],
...
def get_svw(data_root='./dataset/SVW',
               clip_length=8,
               train_interval=2,
               val_interval=2,
               mean=[0.485, 0.456, 0.406],
...
def creat(name, batch_size, num_workers=16, **kwargs):
    ...
    if name.upper() == 'UCF101':
        ...
        train, val = get_hollywood2(**kwargs)
    elif name.upper() == 'MSR_VTT':
        train, val = get_msr_vtt(**kwargs)
    elif name.upper() == 'SVW':
        train, val = get_svw(**kwargs)
    else:
